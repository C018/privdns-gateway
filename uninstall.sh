...
systemctl disable --now pdg-bot pdg-probe81 mosdns sing-box mihomo pdg-mitm pdg-rules-update.timer pdg-health.timer 2>/dev/null || true
...
if [[ "${1:-}" == "--purge" ]]; then
  ...
  rm -f /usr/local/bin/mosdns /usr/local/bin/sing-box /usr/local/bin/mihomo \
